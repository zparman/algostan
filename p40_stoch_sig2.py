...
def main():
    ...
    for i, row in tickers.iterrows():
        ...
        df_stoch_sig2  = get_stoch_sig2(symb,df_ta)
...
def get_stoch_sig2(s,df0):
    outfile = OUTFDIR+s+'_stoch_sig2'+YNN
    ...
    for i in range(1,len(df0)):
        ...
        if  (   (df0.at[prev,'stokk'] <= df0.at[prev,'stokd'])  
            ...
            and (df0.at[prev,'stokk'] <= 40) 
            and (df0.at[curr,'stokk'] >  40)
            ):
                d = df0.at[curr,'Date']
                print(d+","+s)      
    sys.stdout.close()
...
def get_pnl(symb,date):
    infile = OUTFDIR+s+'_stoch_sig2'+YNN
    df1 = read_csv(infile)
    print(infile)
